...
factor.vec2=c(.5,1,1.5,2,2.5,3,3.5,4,4.5,5,6,7,8,10,12) ##work with this right now 
burn=2500
post=5000


##Generate Output Data Frames
output.30.1=bart_loop(tf.exp=tf.exp.30,tf.beta=tf.beta.1,factor.vec=factor.vec2,
                      tf.size.vec=tf.size2,n.obs=n30,
                      n.tree.vec=n.tree.vec,burn_size=burn,
                      post_size=post,thin.size=2,sigest=NA)

output.50.1=bart_loop(tf.exp=tf.exp.50,tf.beta=tf.beta.1,factor.vec=factor.vec2,
                      tf.size.vec=tf.size2,n.obs=n50,
                      n.tree.vec=n.tree.vec,burn_size=burn,
                      post_size=post,thin.size=2,sigest=NA)

output.100.1=bart_loop(tf.exp=tf.exp.100,tf.beta=tf.beta.1,factor.vec=factor.vec2,
                      tf.size.vec=tf.size2,n.obs=n100,
                      n.tree.vec=n.tree.vec,burn_size=burn,
                       post_size=post,thin.size=2,sigest=NA)

output.200.1=bart_loop(tf.exp=tf.exp.200,tf.beta=tf.beta.1,factor.vec=factor.vec2,
                      tf.size.vec=tf.size2,n.obs=n200,
                      n.tree.vec=n.tree.vec,burn_size=burn,
                       post_size=post,thin.size=2,sigest=NA)

output.300.1=bart_loop(tf.exp=tf.exp.300,tf.beta=tf.beta.1,factor.vec=factor.vec2,
...
save(output.30.1,file="bart301_v3.R")
save(output.50.1,file="bart501_v3.R")
save(output.100.1,file="bart1001_v3.R")
save(output.200.1,file="bart2001_v3.R")
save(output.300.1,file="bart3001_v3.R")
